            ...
            {dados.map((item) => (
                ...
                <div className="grid grid-rows-1 gap-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-0 border border-black">
                        ...
                        {/* categorias */}
                        <div className={`bg-blue-500 text-white p-1 `}>Categoria:{item.category}</div>
                        <div className="bg-white text-black p-4">Tema: {item.tema}</div>
                        <div key={item.id} className={`bg-gray-200 text-black p-4 
                            ${linhaOculta === item.id ? 'hidden' : 'block'}`}>
                            {item.descricao}
                        </div>

                        {/* view */}
                        <div className="p-4">
                            <ArrowDownUp color="#03541b" onClick={() => setLinhaOculta(item.id )} />
                        </div>
